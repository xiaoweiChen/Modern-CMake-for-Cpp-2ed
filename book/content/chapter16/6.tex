当我们讨论每个特定阶段预设的一般字段时，提到了 condition 字段。现在是回到这个话题的时候了。condition 字段可以启用或禁用一个预设，在与工作流集成时揭示其真正的潜力。本质上，它允许我们在某些条件下绕过不适合的预设，并创建替代的预设。
...
这个条件在系统为 Linux 或 Windows 时评估为真。
