编写单元测试从技术上讲，不使用任何类型的框架是可能的。我们所要做的就是创建我们想要测试的类的实例，执行其一个方法，并检查新的状态或返回的值是否符合我们的预期。然后，我们报告结果并删除被测试的对象。让我们试一试。
...
CTest执行了两个测试，并报告其中一个失败了——Calc::Multiply返回的值不符合预期。非常好。我们现在知道我们的代码有一个错误，需要有人修复它。

\begin{myNotic}{Note}
您可能已经注意到，在迄今为止的大多数示例中，我们并不一定采用了第4章“设置您的第一个CMake项目”中描述的项目结构。这样做是为了保持简洁。这一章讨论了更高级的概念；因此，使用完整结构是合理的。在您的项目（无论多小）中，最好从一开始就遵循这种结构。正如一位智者所说：“你踏上这条路，如果你不保持警惕，就不知道会被带到哪里。”
\end{myNotic}

我希望现在已经很清楚，为您自己的项目从头开始构建测试框架是不可取的。即使是最基本的示例也难以阅读，有很多开销，并且没有增加任何价值。然而，在我们采用单元测试框架之前，我们需要重新考虑项目的结构。
